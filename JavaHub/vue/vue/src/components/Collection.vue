<template>
  ...
  <div>
    <DataTable :value="products" responsiveLayout="scroll" v-model:expandedRows="expandedRows"
               ...
               :globalFilterFields="['collecid','exerques','exertype','knowpoint','remarks']"
               editMode="cell" @cell-edit-complete="onCellEditComplete" class="editable-cells-table">
      <template #header>
        <div class="flex align-items-center">
          <span class="p-input-icon-left">
            <i class="pi pi-search" />
            <InputText v-model="filters['global'].value" placeholder="搜索收藏夹" />
          </span>
        </div>
      </template>
      <Column field="expander" style="min-width:20px" :expander="true">
      </Column>
      <Column field="collecid" style="max-width:0px" hidden="true">
        <template #body="slotProps">
          {{slotProps.data.collecid}}
        </template>
      </Column>
    ...
    <Dialog v-model:visible="deleteProductDialog" :style="{width: '450px'}" header="确认删除" :modal="true">
      <div class="flex align-items-center justify-content-center">
        ...
        <span v-if="product">确定要从收藏夹中删除这道题目吗？</span>
      </div>
      <template #footer>
...
export default {
  ...
  mounted() {
    this.request.post("/collecset/select",this.user).then(res => {
      this.products=res
      console.log(res)
    })
  },
  methods: {
    onCellEditComplete(event) {
      ...
      if (newValue.trim().length > 0)
      {
        ...
        let obj=JSON.stringify({
          "collecid":data.collecid,
          "remarks":newValue
        })
        this.request.post("/collecset/update",obj).then(res => {
          if(res){
    ...
    deleteProduct() {
      this.products = this.products.filter(val => val.collecid !== this.product.collecid);
      let obj=JSON.stringify({
        "collecid":this.product.collecid
      })
      this.request.post("/collecset/delete",obj).then(res => {
        if(res){
