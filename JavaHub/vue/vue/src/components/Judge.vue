<template>
  <div>
    ...
    <div class="card">
      <h4>判断题</h4>
      <component :is="SingleCh" :data="data" ref="singleCh" @answer="submit2"></component>
    </div>
    <Toolbar class="mb-4">
...
export default {
  ...
  mounted() {
    ...
    this.request.post("/exercise/type",obj).then(res => {
      ...
      this.checkLast()
      this.SingleCh=defineAsyncComponent(() => import("@/pattern/Judge"))
    })
  },
  methods:{
    newComment(){
    ...
    reload () {
      ...
      this.comments=this.data.comments
      this.exerid=this.data.exerid
      document.getElementById("ans").innerText="参考答案："+this.data.exerques
      document.getElementById("kp").innerText="知识点："+this.data.knowpoint
      this.checkLast()
    },
    getTime(time){
