<template>
  <div>
    ...
    <div class="card">
      <h4>多选题</h4>
      <component :is="SingleCh" :data="data" ref="multiCh" @answer="submit2"></component>
    </div>
    <Toolbar class="mb-4">
...
export default {
  ...
  mounted() {
    ...
    this.request.post("/exercise/type",obj).then(res => {
      ...
      document.getElementById("ans").innerText="参考答案："+this.data.answer
      document.getElementById("kp").innerText="知识点："+this.data.knowpoint
      this.checkLast()
      this.SingleCh=defineAsyncComponent(() => import("@/pattern/MultiCh"))
    })
  },
  methods:{
    newComment(){
    ...
    submit(){
      ...
      this.$refs.multiCh.getMsg()
      this.hidden=false;
    },
    submit2(answer){
      let ans = this.data.answer//正确答案
      let flag=true
