...
parser.add_argument('-train_data_path', type=str, default=r'E:\javahub\JavaHub\data.csv',
...
def run_epoch(m, optimizer, students, batch_size, num_steps, num_skills, training=True, epoch=1):
    ...
    while (index + batch_size < len(students)):
        ...
        for i in range(batch_size):
            ...
            problem_ids = reg(student[1])
            correctness = reg(student[2])
            # 答题序列的前n-1个作为模型的输入
            for j in range(len(reg(problem_ids)) - 1):
        ...
        if training:
            ...
            # 将输出层转化为一维张量
            output1 = output.contiguous().view(-1)
            # tf.gather用一个一维的索引数组，将张量中对应索引的向量提取出来
            logits = torch.gather(output1, 0, target_id)
            # 预测, preds是0~1的数组
...
def main():
    ...
    train_students, train_max_num_problems, train_max_skill_num, real_skill_list = load_data(train_data_path)
    ...
    for i in range(args.epochs):
        rmse, auc, r2, output = run_epoch(model, optimizer, train_students, batch_size, num_steps, num_skills, epoch=i)
        # print(rmse, auc, r2, torch.sigmoid(output[-1, :]))
        # 最后一代r
        if i == args.epochs - 1:
            probs = torch.sigmoid(output[-1, :])
            print(real_skill_list)
            li = []
            for k in probs:
                li.append(k.item())
            print(li)
            with open(r"E:\javahub\JavaHub\predict.csv", 'w', newline='', encoding='UTF-8') as f:
                writer = csv.writer(f)
                writer.writerow(real_skill_list)
                writer.writerow(li)
            print("OK")
